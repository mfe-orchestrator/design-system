...
const SelectContent = React.forwardRef<
    ...
    React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content> & VariantProps<typeof selectContentVariants> & TestIdProps
>(({ className, children, position = "popper", dataTestId, ...props }, ref) => {
    ...
    return (
        <SelectPrimitive.Portal>
            <SelectPrimitive.Content
                ref={ref}
                className={cn(selectContentVariants({ layoutSize, fullWidth, position }), className)}
                position={position}
                {...props}
